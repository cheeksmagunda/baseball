...
3. **T-65 Final Run** (Exactly at T-65):
   - Monitor wakes up and runs the FULL pipeline:
     ...
     - **Phase 1: RotoWire expected lineups** (required) — populates `batting_order` from beat-reporter projections, sets `batting_order_source` to `"rotowire_confirmed"` or `"rotowire_expected"`. Raises RuntimeError if unreachable or returns zero games — treated as required infrastructure identical to the Odds API.
...
Old historical-into-DB seed paths (`app/seed.py`, the `draft_lineups` / `draft_slots` / `weight_history` tables) were removed.  The DB now stores **only current-cycle live state** — the T-65 monitor populates `slates` / `slate_games` / `slate_players` / `player_stats` / `player_game_log` / `team_season_stats` / `cached_lineups` / `player_scores` / `score_breakdowns` from live MLB API + Savant calls.  Migration `d4e5f6a7b8c9` drops the orphaned historical tables.
